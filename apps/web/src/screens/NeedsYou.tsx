...
import { ACTIONS_QK, approveAction, declineAction, goAction, orderActions } from "@/lib/actions";
...
export default function NeedsYou({ userId }: { userId: string }) {
  ...
  // The whole outbox, so change-request and action patches can be overlaid too;
  ...
  // (AQ-4): a reeve action's Go drafts one, and it appears here. A pending
  ...
  // One resilient realtime channel (F9) for the three tables this screen reads.
  useRealtimeChannel({
    channel: "needs-you-changes",
    userId,
    subscriptions: [
      { table: "actions", onChange: () => void qc.invalidateQueries({ queryKey: ACTIONS_QK }) },
      {
        table: "captures",
